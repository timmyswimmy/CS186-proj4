...
public class JoinOptimizer {
    ...
    public double estimateJoinCost(LogicalJoinNode j, int card1, int card2,
        ...
        if (j instanceof LogicalSubplanJoinNode) {
            ...
            // join.
            return card1 * (cost2 + card2) + cost1;
        }
    }
	...
	PlanCache bestJoins = new PlanCache();
	CostCard candidate = new CostCard();
	//if(debug)
	//   return joins;
	if(debug)
	...
	for (int i = 1 ; i <= joins.size() ; i++)
	    for (Set<LogicalJoinNode> ljns : enumerateSubsets(joins, i))
		{
	       	    double bestCost = Double.MAX_VALUE;
		    for (LogicalJoinNode ljn : ljns)
			{
			    candidate = computeCostAndCardOfSubplan(stats, filterSelectivities, ljn, ljns, bestCost, bestJoins);
			    if (candidate != null)
				{
				    bestCost = candidate.cost;
				    bestJoins.addPlan(ljns, candidate.cost, candidate.card, candidate.plan);
				}
			}
		}
	if (explain)
	    {
		if (candidate != null)
		    printJoins(candidate.plan, bestJoins, stats, filterSelectivities);
	    }
	HashSet<LogicalJoinNode> ans = new HashSet<LogicalJoinNode>(joins);
	if (debug)
	    {
		System.out.println("Ans");
		System.out.println(ans);
		System.out.println("Get Order Ans");
		System.out.println(bestJoins.getOrder(ans));
		System.out.println("END");
	    }
	if (joins.size() == 0)
	    return joins;
	return bestJoins.getOrder(ans);
    }
